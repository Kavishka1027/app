...
function AvailableDogItems() {
  ...
  useEffect(() => {
    const fetchSellItems = async () => {
      try {
        const response = await axios.get('http://localhost:3000/api/sellItem/user/availableDogs');
        setSellItems(response.data.availableDogs);
      } catch (error) {
...
export default AvailableDogItems;
